...
public class BoardDao extends DAO {
	...
	private final String SITSEARCH = "SELECT A.NNAME, A.PIC, B.MAXP, C.LOCATION1, TO_CHAR(C.SDATE,'MM-DD') AS SDATE, TO_CHAR(C.EDATE,'MM-DD')AS EDATE,  C.STIME, C.ETIME,   C.PRICE,  C.STATUS FROM MEMBERS A, SITTER B, TRADEBOARD C WHERE A.ID = B.ID AND B.ID = C.SELLER AND (C.STIME >=?  AND  C.ETIME <= ?) AND B.MAXP=?";
